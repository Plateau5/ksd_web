...
<div class="wrapper" id="wrapper">
	<div class="detail_content">
		...
		<table class="content_box">
			...
			<c:forEach items="${vo.data }" var="bean">
				<tr>
					<td colspan="6">${bean.name }(元)</td>
					<td colspan="6">${bean.amount }</td>
				</tr>
			</c:forEach>
			<tr>
				...
				<td colspan="10" class="approval_flow">
					<table class="flow_lists">
						...
						<c:forEach items="${list }" var="bean">
							<tr>
								<td colspan="10">${bean.modify_name } ${bean.status_value} <c:if test="${not empty bean.remark }">添加评论 (${bean.remark})</c:if> ${bean.modify_time}</td>
							</tr>
						</c:forEach>
						</tbody>
					</table>
