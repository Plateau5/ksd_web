	...
	prototype: {
		...
		showLabel: function( element, message ) {
			...
			if ( label.length ) {
				...
				//label.html(message);
				label.html("<i>&nbsp;</i><span>" + message + "</span>");
			} else {
				// create label
				label = $("<" + this.settings.errorElement + ">")
					.attr("for", this.idOrName(element))
					.addClass(this.settings.errorClass)
					.html("<i>&nbsp;</i><span>" + (message || "") + "</span>");
				if ( this.settings.wrapper ) {
...
jQuery.validator.addMethod("isMobile", function(value, element){
	...
	var m = /^((13[0-9])|(15[^4,\D])|(18[0-9])|(170))\d{8}$/;
	return m.exec(value) ? true : false;
},"手机号格式错误");
jQuery.validator.addMethod("phone", function(value, element){
	...
	var p1=/^[0][1-9][0-9]{1,3}-[0-9]{5,10}$/;
	var p2=/^[1-9]{1}[0-9]{5,8}$/;
	if(value.length>9){
		return p1.exec(value) ? true : false;
	}else{
		return p2.exec(value) ? true : false;
	}
});